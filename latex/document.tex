...
Uma vez que as senhas são vazadas e descobertas, as credencais de acesso estão
...
O artigo "The many ways to hack 2FA" de \cite{article:3} explora as vulnerabilidades e
métodos de burlar o duplo fator de autentiação ou o multifator fator de autentiação,
apesar de serem consideradas mais seguras que as senhas isoladas.
Muitas pessoas ainda podem ser vítimas de e-mails de phishing simples, onde um e-mail de
phishing envia um link fraudulento para um site falso para roubar as credenciais do usuário.
Essencialmente, o invasor engana o usuário para que ele insira suas credenciais em um site falso.

\cite{article:3} cita que um invasor, conhecendo o e-mail e o número de telefone de um
usuário, pode abusar das mensagens de recuperação por SMS usadas em alguns serviços digitais
para multi fator de autenticação.
O invasor pode enviar mensagens SMS fraudulentas se passando por um provedor de serviços,
solicitando um código de "reativação" ou "verificação" de conta.
Se o usuário fornecer o código, o invasor pode usá-lo para realizar autenticação na conta legítima.

Por mais sofisticados que os serviços digitais possam ser, não existe garantias de segurança.
\cite{article:3} destaca que, embora o multi fator de autenticação aumente a segurança,
ele não é infalível e pode ser contornado através de técnicas de phishing para roubar
credenciais ou enganar usuários.

É fundamental mitigar os riscos de segurança por meio de práticas individuais.
